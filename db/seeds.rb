...
Author.create!([
                 ...
                 { name: '米林正明', description: "米林正明
                   ...
                   著書に
                   『Docker入門』(技術評論社)
                   『Facebook発 新プログラミ��グ言語「Hack」スタートアップガイド』(技術評論社)
                   『Javaルールブック』(技術評論社)
                   等。" },
...
Book.create!([
               ...
               { title: 'ラズパイマガジン 2016年6月号 日経BPパソコンベストムック', isbn: 'B01EH2RX42',
                 ...
                 description: "内容紹介
                   ...
                   方法から電子工作ま��を基礎から解説して、幅広い活用法を指南します。
                   特集1「アキバの人気パーツ 配線図ベスト17」では、最大手のパーツショップで最���人気
               ...
               { title: 'アジャイルサムライ−達人開発者への道',
                 ...
                 description: "内容紹介
                   ...
                   そのために私は本書を書いた。もっと��まくソフトウェアを届けるやり方を探し求め、分かちあい、見出していこう（でもあんまり深刻に受け止めすぎないで。楽しみながらやっていこう）。
               ...
               { title: 'ネットワークスペシャリスト 合格テキスト 2016年度 (情報処理技術者試験対策) ',
                 ...
                 description: "内容紹介
                   ...
                   長年にわた��情報処理試験対策の指導実績に基づいて構成しているので
               ...
               { title: '情熱プログラマー ソフトウェア開発者の幸せな生き方',
                 ...
                 description: "内容紹介
                   ...
                   著者略歴 (「BOOK著者紹介情報」より)
                   Fowler,Chad
                   世界的に著名なソフトウェア開発者、トレーナー、マネージャ、演説家、ミュージシャ���。過去10年間に、世界最大級の企業のいくつかで働き、最優秀のソフトウェア開発者という評価を受けた。InfoEther,Inc.のCTOとして、Ruby言語における顧客からの困難な問題の解決に、その時間の多くを割いている。RubyConf、RailsConf、RailsConf Europeの共同主催者であり、多数の人気ソフトウェア書籍の著者または共著者である(本データはこの書籍が刊行された当時に掲載されていたものです)" },
               ...
               { title: 'WEB+DB PRESS Vol.93 ',
                 ...
                 description: "内容紹介
                   ...
                   スケジュール、開発するプロダクトのスコープ、そして開発に参加するメンバーのスキルなど、開発に関わるさまざまなものをどのように見積もるかを解説した特集です。見積りに必要な各種データをどのように集めるのか、そしてどのように見積りに��かすのかも解説します。また、開発の途中で問題が変化するプロジェクトについても、どのように見積りを行い、変化に対応していくのかを解説します。
               ...
               { title: 'マイクロサービスアーキテクチャ',
                 ...
                 description: "内容紹介
                   ...
                   内容（「BOOK」データベースより）
                   柔軟かつシンプルな新時代のソフトウェアアーキテクチャ、マイクロサービスの本質に迫る!モノリシック(一枚岩)からの脱却。複数の小さいマイクロサービスに分割してそれらを連携させることで、迅��なデプロイ、優れた回復性やスケーラビリティを実現する。" },
               ...
               { title: 'Dockerエキスパート養成読本［活用の基礎と実践ノウハウ満載！］ (Software Design plus)',
                 ...
                 description: "内容紹介
                   Docker は、アプリケーショ��の開発・配布・実行のための、オープンソース・コンテナ型仮想化技術です。2013 年の登場以来、重要なITインフラ技術として、エンジニアから、そしてIT ベンダから最重要視されるようになりました。本書では、Docker をソフトウェア開発・運用で活用するために知っておきたい基礎と実践のための知識をわかりやすくまとめてお届けします。その導入にあたってはいまだ試行錯誤の側面もありますが、まずは本書で、その概要と最新動向はもとより、現場で活用するためのノウハウを習得してみてください。
                   内容（「BOOK」データベースより）
               ...
               { title: 'Interface(インターフェース) 2016年10月���',
                 isbn: '',
               ...
               { title: 'システムテスト自動化 標準ガイド CodeZine BOOKS',
                 ...
                 description: "内容紹介
                   ...
                   一方で、ソフトウェアをユーザーが操作したときに問題がないことを確認す���「システムテスト」では、自動化が進んでいません。そのため、顧客やユーザーにソフトウェアを確認してもらう受け入れテストへなかなか移れないケースが多発しています。これは顧客・ユーザーへ動作するソフトウェアをいつでも、何度でも提出し、確認しながら進めていくアジャイル開発において、特に大きなボトルネックになってきます。
               ...
               { title: '実践 Python 3',
                 ...
                 description: "内容紹介
                   ...
                   主なテーマは、エレガントなコーディングのためのデザインパターン、処理速度向上���ための並行処理とCython、高レベルなネットワーク処理、そしてグラフィックスです。
               ...
               { title: 'ハイパフォーマンス ブラウザネットワーキング ―ネットワークアプリケーションのためのパフォーマンス最適化',
                 ...
                 description: "内容紹介
                   ...
                   内容（「BOOK」データベースより）
                   本書は「ブラウザ」に関連する、インターネットで使用される様々なネットワーク技術をまとめたものです。WebSocketやHTTP/2.0やWebRTCのように最新ブラウザで簡単に動作する新しい技���から、そのような技術の土台となるTCPやUDPやトランスポート層についてまで幅広くカバーします。" },
               ...
               { title: 'JavaScriptパターン ―優れたアプリケーションのための作法',
                 ...
                 description: "内容紹介
                   ...
                   内容（「BOOK」データベースより）
                   JavaScriptについては一通りマスターした中上級者がさらに上を目指すための高度でかつ有用なテクニックを盛り込んでいます。デザインパターン、コーディングパターン、アンチパターンに基づき、いままで以上にJavaScriptを使いこなし、プログラミングの力を向上させ、JavaScriptのエキスパートになるための知識と技を��富に紹介。次のステップに踏み出したいプログラマにとって貴重な情報源となる一冊です。Yahoo!のエンジニアとして数々の有用なアプリケーションの開発に携わってきた著者がJavaScriptを200%使いこなすための秘伝を伝授します。" },
               ...
               { title: 'プログラマのためのDocker教科書 インフラの基礎知識＆���ードによる環境構築の自動化',
                 isbn: '',
